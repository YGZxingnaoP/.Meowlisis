# -*- coding: utf-8 -*-
# func/meowsinger/port/gemini.py
# meowsinger 独立 Google Gemini 端口（OpenAI 兼容，支持 function calling 与流式）
import re
...
class MeowSingerGeminiLLM:
    """meowsinger 独立 Google Gemini 客户端，apikey 与参数完全独立"""
    ...
    def __init__(self, config=None):
        ...
        self.client = None
        if not self.api_key:
            self.log.error("meowsinger Google Gemini API Key 未配置")
            return
        try:
            self.client = OpenAI(api_key=self.api_key, base_url=self.base_url)
        except Exception as e:
            self.log.error(f"初始化 meowsinger Google Gemini 客户端失败: {e}")
    ...
    def chat(self, messages, tools=None, tool_choice=None, max_tokens=None, temperature=None):
        ...
        try:
            ...
            self.log.error(f"meowsinger Google Gemini 调用异常: {e}")
            return None
    ...
    def chat_stream(self, messages, tools=None, tool_choice=None):
        ...
        try:
            ...
            self.log.error(f"meowsinger Google Gemini 流式调用异常: {e}")
            return iter([])
