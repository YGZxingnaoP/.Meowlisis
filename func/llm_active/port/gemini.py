# -*- coding: utf-8 -*-
# func/llm_active/port/gemini.py
# 主动回复 Google Gemini 端口（复用 llm 配置，max_tokens 两倍且上限 2048）
...
class AutoGeminiLLM:
    """主动回复 Google Gemini 客户端（OpenAI 兼容，支持 function calling 与流式）"""
    ...
    def __init__(self):
        ...
        self.max_tokens = min(cfg.gemini_max_tokens * 2, 2048)
        self.top_p = cfg.gemini_top_p
        self.enable_thinking = cfg.gemini_enable_thinking

        self.client = None
        if not self.api_key:
            self.log.error("主动回复 Google Gemini API Key 未配置")
            return
        try:
            self.client = OpenAI(api_key=self.api_key, base_url=self.base_url)
            self.log.info(f"主动回复 Google Gemini 客户端初始化成功，模型: {self.model}")
        except Exception as e:
            self.log.error(f"初始化主动回复 Google Gemini 客户端失败: {e}")

    def _reasoning_effort(self, options):
        """构建思考档位（origin 可覆盖开启深度思考）；Gemini 3 无法完全关闭思考"""
        enable = self.enable_thinking
        if options and 'enable_thinking' in options:
            enable = options.get('enable_thinking')
        return "high" if enable else "low"
    ...
    def chat(self, messages: List[Dict], tools: Optional[List[Dict]] = None,
        ...
        params = {
            ...
            "top_p": self.top_p,
            "reasoning_effort": self._reasoning_effort(options),
        }
        if tools:
            params["tools"] = tools
        if tool_choice:
        ...
        try:
            ...
            self.log.error(f"主动回复 Google Gemini 调用异常: {e}")
            return None
    ...
    def chat_stream(self, messages: List[Dict], tools: Optional[List[Dict]] = None,
        ...
        try:
            ...
            self.log.error(f"主动回复 Google Gemini 流式调用异常: {e}")
            return iter([])
