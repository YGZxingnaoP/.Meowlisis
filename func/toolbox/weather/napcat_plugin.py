# -*- coding: utf-8 -*-
# func/toolbox/weather/napcat_plugin.py
# Weather 的 QQ 适配层：负责 QQ 场景的 excuse（角色口吻发 QQ 询问 + 绑定用户等待 + 判断是否另起话题）
# 不负责结果回传（结果由 weather_core 生成后交给 napcat 模块发 QQ，不走 pipeline TTS）
...
@singleton
class TBWeatherNapcatPlugin:
    """Weather 的 QQ excuse 适配层"""

    def __init__(self):
        self.log = DefaultLog().getLogger()

    def ask_city(self, qq_context: Dict, username: str = ""):
        """QQ 场景追问城市。
    ...
    def ask_qq(self, question: str, qq_context: Dict, username: str = "",
        ...
        返回统一结构化结果：
        - ("answer", value)：用户回答了追问，value 为提取的信息；
        ...
        try:
            reply = q.get()  # 直接取用户新的回复（绑定用户，无超时）
        finally:
        ...
        # 4. 判断回复是否回答了追问（另起话题则返回 redeliver）
    ...
    def _in_character(self, question: str, username: str) -> str:
        """以角色身份生成询问（用完整角色提示词 + toolbox LLM）"""
        try:
            ...
            self.log.exception("[WeatherNapcatPlugin] 角色口吻生成询问失败")
        return question
    ...
    def _decide(self, reply: str, question: str, username: str, extract_hint: str):
        ...
        try:
            ...
            self.log.exception("[WeatherNapcatPlugin] 判断回复失败")
        return ("answer", reply.strip())
