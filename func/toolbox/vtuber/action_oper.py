...
class ActionOper:
    ...
    def changeScene(self, sceneName):
        if self.allow_scene(sceneName) == True:
            ...
            # 背景乐切换
            if sceneName in self.obsConfig.song_background:
                song = self.obsConfig.song_background[sceneName]
    ...
    def init_scene(self):
        ...
        # 背景乐切换
        if scene_name in self.obsConfig.song_background:
            song = self.obsConfig.song_background[scene_name]
            self.obs.play_video("背景音乐", song)
            time.sleep(1)
