# -*- coding: utf-8 -*-
# func/toolbox/svg_painter/port/deepseek.py
...
class TBSvgPainterDeepSeekLLM(TBSvgPainterLLMBase):
    ...
    def __init__(self, config):
        ...
        self.client = None
        if not self.api_key:
            self.log.error("[svg_painter] DeepSeek API Key 未配置（请在工具箱-绘画配置中填写）")
            return
        try:
            # 不设硬超时：agent 走流式 + 无数据心跳来感知运行状态，慢/长思考不会被掐断
            self.client = OpenAI(api_key=self.api_key, base_url=self.base_url, max_retries=0)
            self.log.info(f"[svg_painter] DeepSeek 绘画客户端初始化成功，模型: {self.model}")
        except Exception as e:
            self.log.error(f"[svg_painter] 初始化 DeepSeek 绘画客户端失败: {e}")
