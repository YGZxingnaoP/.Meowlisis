# -*- coding: utf-8 -*-
# func/toolbox/svg_painter/port/base.py
# 绘画模型端口基类 + 工厂：统一 OpenAI 兼容接口（thinking 与 function calling 并存）
# 参考 func/llm_active/port 的写法，但配置完全独立（读取 TBSvgPainterConfig）

from typing import List, Dict, Optional


class TBSvgPainterLLMBase:
    """绘画端口统一行为：
...
def create_painter_llm(config):
    ...
    if config.llm_type == "aliyun":
        from func.toolbox.svg_painter.port.aliyun import TBSvgPainterAliyunLLM
        return TBSvgPainterAliyunLLM(config)
    from func.toolbox.svg_painter.port.deepseek import TBSvgPainterDeepSeekLLM
    return TBSvgPainterDeepSeekLLM(config)
