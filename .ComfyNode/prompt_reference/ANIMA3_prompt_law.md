...
| 2    | OUTPUT PROTOCOL        | 输出格式硬规则（tag 锚点段一行、全小写、禁止什么；自然语言主描述 3-6 句） |
...
| 禁止输出     | 质量词 (masterpiece/best quality/score_X 等)、画师名 (@artist)。环境天气/时辰/光照一律写进自然语言主描述，不写成 tag |
| 输出形式     | 纯文本一行，无 code fence、无 markdown、无引导语             |
| 自然语言主描述 | **Anima3 的强项，也是主要输出手段**：用 3-6 句英文自然语言写清 主体/动作/环境/**光照**/镜头/氛围（200-400 字符），置于所有 tag 之后（prompt 末尾）。**其中光照必写**：用一句说清光源与整体明暗，具体写什么由题材决定。tag 段退居辅助：只保留 8-14 个锚点（主体、角色辨识特征、景别），≤150 字符。多人角色归属、复杂构图、特殊姿势、分镜关系**一律用自然语言**表达 |
...
| 5    | **光照已写**   | 自然语言主描述里必须含一处光照描写（写什么、多亮由题材决定）  |
...
### 4.3 视角/视线多样性规则（禁止连续同构）

**单人场景**：先按 §11.1~§11.4 选镜头，再从下列维度各取 1 项；**同类需求不得连续两张使用同一组合**（构图要有新意）。

| 维度 | 备选（全部取自 §11） |
| ---- | -------------------- |
| 景别 | `close-up` / `upper body, bust shot` / `cowboy shot` / `full body` / `wide shot` |
| 角度 | `from front` / `from side, profile` / `from behind, back view` / `from above` / `from below` / `over shoulder` / `dutch angle` / `foreshortening` |
| 视线 | `looking at viewer` / `looking away` / `looking back, over shoulder` / `looking to the side` / `eyes closed` |

`direct eye contact, facing viewer` **不是默认值**：仅当「角度=from front 且 视线=looking at viewer」被抽中时才写。
角度与视线须随场景语义轮转（例：仰拍感 → `from below`；读书/低头 → `looking down`；离去/背影 → `from behind, facing away`；对峙 → `dutch angle`）。

**两人及以上场景**：不强制注入 `direct eye contact`。根据角色间互动关系选择合适的视线标签（如 `looking at another`），或由用户明确指定。

| 用户意图            | 适用  | 输出                                 |
| ------------------- | ----- | ------------------------------------ |
| 未指定（单人）      | solo  | 从上方三维度池各取 1 项并轮转        |
| 正面（用户明说）    | solo  | `direct eye contact, facing viewer`  |
...
### 4.4 自然语言主描述（Anima 的强项）与 tag 锚点

**核心原则**：**自然语言为主、tag 为锚**。Anima3 是 DiT 模型，最吃完整句子的画面描述——
主描述用 3-6 句英文（200-400 字符）依次写：主体与动作/姿态 → 表情与情绪 → 场景环境 → 光线与色调 →
镜头角度/构图/相机距离 → 氛围。tag 段只保留 8-14 个必须锚定的项（主体、角色辨识特征、景别），≤150 字符。
**自然语言段落统一放在 prompt 末尾（所有 tag 之后）**，与 tag 用逗号分隔。

**必须用自然语言（而不是堆 tag）的场景**：
...
**格式规则**：
- 主描述 3-6 句完整英文短句，写"镜头里看得见的东西"，合计 200-400 字符
- 一个短句解决一个歧义，宁具体勿堆砌；不要重复 tag 段已锚定的属性
- 自然语言段落统一放在 prompt 末尾（所有 tag 之后），与 tag 用逗号分隔
...
> 以下环境天气和时辰描写可用；光照的具体样子（光源/明暗）写在自然语言主描述里。
...
> 对应槽位：`[detail/mood]`
> 画面「看起来像什么」和「给人什么感觉」——不包含场景地点（§12）和镜头（§11）。以下氛围与特效标签可按需选用。
...
光照**不写进 tag 段**，写在自然语言主描述里（见 §2）：一句英文说清光源与整体明暗即可，具体写什么由题材决定。

§13.3 的光学/摄影标签（`depth of field`、`bloom`、`silhouette`、`soft focus`、`vignette`、`lens flare` 等）、§12.3 的天气/时辰标签照常可用。
