...
class AP_BWE:
    def __init__(self, device, DictToAttrRecursive, checkpoint_file=None):
        ...
        self.amp_pha_stft = amp_pha_stft
        self.amp_pha_istft = amp_pha_istft
    ...
    def __call__(self, audio, orig_sampling_rate):
        with torch.no_grad():
            ...
            amp_nb, pha_nb, com_nb = self.amp_pha_stft(audio, self.h.n_fft, self.h.hop_size, self.h.win_size)
            amp_wb_g, pha_wb_g, com_wb_g = self.model(amp_nb, pha_nb)
            audio_hr_g = self.amp_pha_istft(amp_wb_g, pha_wb_g, self.h.n_fft, self.h.hop_size, self.h.win_size)
